...
function isPackageBuildCurrent(targetDir) {
  ...
  if (!declaredPackageArtifactsExist(targetDir, targetPackageJson)) return false;
  ...
  if (!localDependencyLinksCurrent(targetDir)) return false;
  return true;
}

function localDependencyLinksCurrent(targetDir) {
  const targetPackageJsonPath = path.join(targetDir, 'package.json');
  if (!fs.existsSync(targetPackageJsonPath)) return true;
  const targetPackageJson = JSON.parse(fs.readFileSync(targetPackageJsonPath, 'utf8'));
  for (const dependency of readLocalDependencies(targetPackageJson)) {
    const dependencyDir = localPackageDir(dependency);
    if (!dependencyDir || dependencyDir === targetDir) continue;
    const linkPath = path.join(targetDir, 'node_modules', ...dependency.split('/'));
    const expected = path.relative(path.dirname(linkPath), dependencyDir);
    try {
      const stat = fs.lstatSync(linkPath);
      if (!stat.isSymbolicLink() || fs.readlinkSync(linkPath) !== expected) return false;
    } catch (error) {
...
function declaredPackageArtifactsExist(targetDir, pkg) {
  const artifacts = new Set([pkg.main, pkg.types]);
  collectExportArtifacts(pkg.exports, artifacts);
  for (const artifact of artifacts) {
    if (typeof artifact !== 'string' || !artifact.startsWith('./dist/')) continue;
    if (!fs.existsSync(path.join(targetDir, artifact))) return false;
  }
  return true;
}

function collectExportArtifacts(value, artifacts) {
  if (typeof value === 'string') {
    artifacts.add(value);
    return;
  }
  if (!value || typeof value !== 'object') return;
  for (const nested of Object.values(value)) collectExportArtifacts(nested, artifacts);
}
